...
##returns a list of files,directories,file paths, shared between mod folders/internal folders
static func get_files_at(dir:String,dir_typing:open_type = open_type.DIR) -> PackedStringArray:
	...
	for mod in mod_list:
		if !mod.enabled: continue
		var path = dir if mod.id == 'internal' else join_mod_path(dir,mod.path)
		if !DirAccess.dir_exists_absolute(path): continue
		
		match dir_typing:
			open_type.DIR:
				for folder in DirAccess.get_directories_at(path):
					if folder in files: continue
					files.append(folder)
			open_type.FILES:
			...
			open_type.FILES_PATH:
				for file in DirAccess.get_files_at(path):  files.append(path.path_join(file))
	return files
